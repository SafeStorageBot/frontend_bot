...
export class VerifyComponent implements OnDestroy {
  ...
  decrypt(passkeys: string) {
    this.verifyService
      .decrypt(this.forDecrypt || [], passkeys)
  ...
  encrypt(passkeys: string) {
    this.edit = false;
    this.verifyService
      .encrypt(this.forEncrypt || [], passkeys)
      .pipe(
        switchMap((items: Array<PasswordItem>) =>
          combineLatest(...items.map((i) => this.listService.add(i)))
        ),
      ...
      .subscribe((resp: Array<BotHttpResponse<any>>) => {
        ...
        if (resp.find((item) => item.error)) {
          this._snackBar.open(
            'Error during add ecrypted password on server',
            'Ok',
            {
              duration: 1500,
